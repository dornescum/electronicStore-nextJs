...
const PhoneId = ({itemId}) => {
    ...
    const {
        title, price, id, desc, subtitle, main_img, short_description, battery_size, adaptor, bluetooth,
        manufacturer, os, seller, reviews, tag
    } = itemId;


    // const fetchComments = async () => {
    //     const res = await fetch(`/api/reviews/${itemId.id}`);
    ...
    return (
        <div className='font-dosis mb-20' id={id}>
		  ...
		  h-96 md:h-80 lg:h-96 drop-shadow-md hover:drop-shadow-2xl ease-in duration-300">
                            <Image src={mainImg} alt={title} width='1400' height='1400'
                                   className="object-cover h-96 w-96 cursor-wait"/>
                        </div>
                    }
                </div>
                <div className="basis-1/2 flex flex-col items-start justify-center h-96">
                    <div className='pl-0 ml-0 md:pl-6 lg:mr-60'>
                        <div className=''>
                            <Link href="/tablet">
                                <a className='mt-0 ml-8 bg-zinc-100 px-4'>Tablets</a>
                            </Link>
                        </div>

                        {/*{!itemId && <div className='mt-20 text-red-700 text-3xl'>Loading...</div> }*/}
                        <SingleProduct title={title} price={price} subtitle={subtitle} short_desc={short_description}
                                       onAdd={onAdd}
...
let url = 'https://electronis-api.herokuapp.com/api/tablets/';
